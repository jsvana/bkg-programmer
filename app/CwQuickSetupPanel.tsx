...
//   F Lock  → f_lock = FCC HAM (US 2 m / 70 cm TX limits)
...
// F-Lock dropdown options + default. Labels and values come straight from
// the freq_lock_settings schema module so they can't drift from what the
// codec will accept. Default is FCC HAM — the US 2 m / 70 cm TX band plan.
const F_LOCK_OPTIONS: ReadonlyArray<string> = fLockValues.map((v) => v.label);
const DEFAULT_F_LOCK_LABEL =
  fLockValues.find((v) => v.value === F_LOCK_FCC)?.label ?? F_LOCK_OPTIONS[0]!;
...
function CwQuickSetupInner({
  ...
  const [fLock, setFLock] = useState<string>(DEFAULT_F_LOCK_LABEL);
  ...
  const cfg = useMemo<BkgConfig | null>(() => {
    ...
    return {
      ...
      notes:
        ...
        `${channelIndex} (rx/tx freq, modulation, tx_power, name), the ` +
        "global nr7y_cw_settings block (keyer mode, WPM, key input, " +
        "break-in, byte-2 validity marker), and the F-Lock TX frequency " +
        "lock (freq_lock_settings).",
      ...
      settings: {
        ...
        // TX frequency lock (gSetting_F_LOCK at 0xA150 byte 0). Default
        // FCC restricts TX to the US 2 m / 70 cm amateur bands.
        freq_lock_settings: {
          f_lock: fLock,
        },
      },
    };
  }, [formError, channelIndex, channelName, freqMhz, txPower, keyerMode, wpm, keyInput, fLock]);
